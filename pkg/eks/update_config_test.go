...
var _ = Describe("EKS API wrapper", func() {
	Describe("can update cluster configuration for logging", func() {
		...
		It("should handle unknown facilities", func() {
			cfg.CloudWatch.ClusterLogging.EnableTypes = []string{"anything", "anyOtherThing"}
		...
		It("should expand [] to none", func() {
			...
			Expect(clusterLogging[1].Types).To(Equal(aws.StringSlice(api.SupportedCloudWatchClusterLoggingTypes())))
		})
		...
		It("should expand '*' to _all_", func() {
			...
			Expect(clusterLogging[0].Types).To(Equal(aws.StringSlice(cfg.CloudWatch.ClusterLogging.EnableTypes)))
